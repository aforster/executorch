...
is what dominates the repo's artifact storage.
    ...
    stash <dir>    Replace every *.ptd under <dir> with a <name>.sha256 pointer
    ...
    restore <dir>  Download the blob behind every *.sha256 under <dir>, verify
                   the digest and delete the pointer.
...
else that lands in the artifact directory can end up in the public bucket.
...
# written, regardless of use; re-uploading older blobs keeps hot ones alive.
REFRESH_AGE_DAYS = 80
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if actual.hexdigest() == digest:
                ...
                return
            error = f"sha256 mismatch, got {actual.hexdigest()}"
...
def restore(root):
    for pointer in sorted(root.rglob("*" + POINTER_SUFFIX)):
        fields = pointer.read_text().split()
        stem = pointer.name[: -len(POINTER_SUFFIX)]
        if len(fields) != 2 or not DIGEST_RE.fullmatch(fields[0]) or fields[1] != stem:
